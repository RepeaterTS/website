  ...
  methods: {
    async fetch() {
      ...
      const [downloads, stars, contributors] = await Promise.all([
        fetch('https://api.npmjs.org/downloads/range/2013-08-21:2100-08-21/repeaterts').then(json, noop),
        fetch('https://api.github.com/repos/repeaterts/levers').then(json, noop),
        fetch('https://api.github.com/repos/repeaterts/levers/stats/contributors').then(json, noop),
      ]);
